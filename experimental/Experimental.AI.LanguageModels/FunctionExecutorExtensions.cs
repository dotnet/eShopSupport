...
{
    public static void UseStandardFunctionExecution(this ChatPipelineBuilder builder)
    {
    ...
    private class StandardFunctionExecutionMiddleware : ChatMiddleware
    {
        ...
        public override async IAsyncEnumerable<ChatMessageChunk> CompleteChatStreamingAsync(IChatHandler next, IReadOnlyList<ChatMessage> messages, ChatOptions options, [EnumeratorCancellation] CancellationToken cancellationToken)
        {
            ...
            for (var iteration = 1; iteration <= maxIterations; iteration++)
            {
                ...
                await foreach (var chunk in next.CompleteChatStreamingAsync(messages, options, cancellationToken))
                {
                ...
                if (toolCalls.Any())
                {
                    ...
                    {
                        await next.ExecuteChatFunctionAsync(toolCall, options);
                    }
